...
export default {
    ...
    async mounted() {
        this.loading = true;
        try {
            await this.getAuthProfile(); // 1. ดึง Profile ก่อนเพื่อรู้ว่าใคร Login
            await this.fetchIssue();     // 2. ดึง Issue
        } finally {
            this.loading = false;
        }
    },
