    ...
    <div class="page-header compact-header">
      <div class="header-content">
        <div class="header-left">
          ...
          <div class="title-row">
            ...
            <a-tag :color="getStatusColor(issue.status?.code)" class="status-tag">
              <template #icon>
                <component :is="getStatusIcon(issue.status?.code)" :spin="issue.status?.code === 'inProgress'" />
              </template>
        ...
        <div class="header-actions">
          <a-button @click="$router.go(-1)" class="btn-back">
            <template #icon>
              <ArrowLeftOutlined />
            </template> Back
          </a-button>
              ...
              <div class="alert-content">
                ...
                <div v-if="issue.tester" class="alert-meta">
                  <a-avatar size="small" :src="issue.tester.avatar" style="margin-right: 6px;">
                    <span v-if="!issue.tester.avatar">{{ issue.tester.user_name?.[0]?.toUpperCase() }}</span>
                  </a-avatar>
              ...
              <a-card :bordered="false" class="main-card side-card mb-3">
                ...
                <div v-if="!issue.assignee">
                  <div class="unassigned-state">
                    ...
                    <a-button type="primary" block size="large" class="btn-claim" :loading="actionLoading"
                      @click="goToDevDetail">
                      ✋ รับงาน (Claim)
                    </a-button>
                  </div>
                ...
                <div v-else>
                  <div class="assignee-header">
                    ...
                    <div class="assignee-details">
                      <span class="label">ผู้รับผิดชอบ (You)</span>
                      <h4 class="name text-ellipsis">{{ issue.assignee.user_name }}</h4>
                    </div>
                  </div>

                  <div class="action-area">
                    <a-button v-if="issue.status?.code === 'rejected'" type="primary" block class="btn-fix"
                      @click="goToDevDetail">
                      <ToolOutlined /> แก้ไขงาน (Fix)
                    </a-button>
                    <a-button v-else-if="issue.status?.code === 'received'" type="primary" block class="btn-start"
                      @click="goToDevDetail">
                      <SyncOutlined /> เริ่มงาน (Start)
                    </a-button>
                    <a-button v-if="isReporter" type="dashed" block style="margin-top: 8px;" @click="goToEditDetail">
...
/* ✅ Class สำหรับกล่องเนื้อหาฝั่งซ้าย (บังคับความสูง) */
.content-card {
  min-height: 600px;
}
